...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:8000/teacher/homepage/student-count')
    ...
    axios.get('http://localhost:8000/teacher/business-count')
      .then(response => setBusinessesCount(response.data.count))
      .catch(error => console.error('Error fetching businesses count:', error));

    // Fetch data for the number of positions
    axios.get('http://localhost:8000/teacher/count-positions')
      .then(response => setPositionsCount(response.data.count))
      .catch(error => console.error('Error fetching positions count:', error));
  }, []); // Empty dependency array ensures the effect runs once on mount

  return (
    <div className="dashboard">
      <div className="info-box">
        <div className="info-text">Number of Students:</div>
        <div className="info-number">{studentsCount}</div>
      </div>
      <div className="info-box">
        <div className="info-text">Number of Businesses:</div>
        <div className="info-number">{businessesCount}</div>
      </div>
      <div className="info-box">
        <div className="info-text">Number of Recruitment Positions:</div>
